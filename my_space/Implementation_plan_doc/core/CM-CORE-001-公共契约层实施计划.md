...
- `src/codemigrator/core/resources/`：`phase-tool-policy/v2`、`verification-policy/v1`、`session-budget/v1` 和 `session-templates/v1` 数据文件及其 manifest。
...
- `StableErrorCode` 集中包含 M-00/M-02/M-05/M-07/M-10/M-12 已登记的稳定码，包括工具/路径/读写/查询/Shell/Exec、Spec/descriptor、PLAN 八码、验证集合四码及 API 幂等码；不把 `FailureReason` 成员并入其中。
...
- [x] 编写 URI 可寻址、JSON UTF-8、资源版本、SHA-256 manifest 和 missing resource 拒绝测试。
...
- [x] 运行 `PYTHONPATH=src:/tmp/codemigrator-core-venv/lib/python3.12/site-packages python3 -m pytest tests/core tests/contracts -q`，49 passed。
...
- [x] 做静态审查：core 不导入 `runtime`、不读取环境变量、不创建线程/任务、不定义 API 事件常量。
...
| 资源摘要 | `PYTHONPATH=src pytest tests/contracts/test_policy_resources.py -q` | URI、内容和 manifest 摘要一致 | D-04、CM-VERIFY D-04、CM-WORKSPACE D-02、CM-MEMORY D-01 |
...
- [x] core 模块迭代记录按模板生成。
- [x] 主任务表 CM-CORE-001 更新为已完成并带验证证据。
