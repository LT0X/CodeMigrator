...
> 文档状态：V4 当前架构基线；sandbox-worker 是全部检查执行的唯一不可信执行面——Run actor 内部验证与输出基线 Scaffold 共用同一条 UDS 派发链与一次性 overlay 纪律（消费者恰为两项；`CheckRunner` 已作为 Agent 工具退役，M-12 D-034：会话自检并入 Shell）。沙箱生命周期从"单次检查"延展为"Slice 全生命周期"（Slice 创建→集成/废弃）：每 Slice 候选工作区即长驻沙箱卷，构建缓存与已装依赖跨命令驻留（迭代加速），集成 receipt 后销毁工作区与沙箱。  
> 技术范围：app↔worker 本地协议（`ExecuteCheck` 冻结方法面）、active-attempt 接纳、目标工具链镜像挂载、bubblewrap 与 default-deny seccomp、长驻沙箱卷与 Shell 执行面（受控网络出口）、Exec 嵌入式引擎资源开销、资源限制与进程组回收；部署基线 Linux kernel ≥5.15、cgroup v2、bubblewrap ≥0.8、user namespace 可用。  
> 契约真相：`CheckCommandTemplate`、`CheckAction`、`ExecutionSubject`、`CheckStatus` 与超时/输出默认档由 [M-00](CodeMigrator_垂类设计原则与架构哲学.md) 所有；本地 worker 协议由 `codemigrator-sandbox` crate 所有；验证语义与 fingerprint 由 [M-10](CodeMigrator_验证引擎.md) 所有；`Shell`/`Exec` 工具面与 checkpoint 批量校验由 [M-12](CodeMigrator_工具系统与Hook.md) 与 [候选工作区与工具网关](CodeMigrator_候选工作区与工具网关.md) 所有；overlay 来源与 Scaffold 执行时机由 [候选工作区与工具网关](CodeMigrator_候选工作区与工具网关.md) 与 [Git 集成](CodeMigrator_工作空间与Git集成.md) 所有。  
> 关联文档：[crate 与协议归属](CodeMigrator_核心目录架构设计.md)、[Run actor 与接管](CodeMigrator_Harness总体设计.md)、[验证引擎](CodeMigrator_验证引擎.md)、[工具路径安全](CodeMigrator_工具系统与Hook.md)、[会话与修正](CodeMigrator_会话与运行时修正编排.md)。

沙箱面对的是不可信代码：翻译后的测试程序、依赖树、构建脚本与目标工具链内的任意可执行内容都可能读取宿主秘密、联网或留下后台进程。`sandbox-worker` 是这些执行的唯一服务端：Run actor 的 `InternalVerificationDispatch` 与 Harness 在输出基线初始化时执行的 Scaffold 经同一条 UDS 派发链进入同一套 bubblewrap 隔离；Agent 的 `Shell` 调用则直接在本 Slice 专属长驻沙箱卷内执行（[M-12](CodeMigrator_工具系统与Hook.md)，工作区即沙箱卷，M-08），不经 UDS 派发链——`CheckRunner` 已退役，UDS 派发链上不再存在任何由模型工具触发的消费者。

裁决层检查命令只有一个来源：Run 创建时冻结的目标端工具链描述符 `CheckCommandTemplate`，由 Harness 以冻结参数实例化为 `program + argv[]`。模型零输入——不提供 program、argv、shell 片段或环境变量；命令面之外的任何执行请求在进入本执行面前已被拒绝且零派发（M-00/M-12）。worker 在固定隔离边界内运行该命令，捕获执行事实并清空整个进程组；它不判断迁移是否正确，也不拥有 Run、Git 或 PostgreSQL。`InternalVerificationDispatch` 不是模型工具，不在 phase-tool policy 中注册。执行面自此分层（M-12 D-034 重写）：裁决层冻结通道保确定性——fingerprint 的计算输入只有冻结检查集与 tested_commit overlay；`Shell` 通道保能力——语言无关的自由执行，不经冻结模板；验证只走前者。一次性 overlay 纪律适用于裁决层（tested_commit overlay），Shell 在长驻沙箱卷内的写效果由 checkpoint 批量校验兜底（M-08）。
...
pub struct OverlayId(uuid::Uuid);

#[serde(tag = "origin", rename_all = "SCREAMING_SNAKE_CASE")]
pub enum CheckSubject {
    Internal { subject: ExecutionSubject, check_id: CheckId },
    BaselineInit,
}

pub struct FrozenCheckCommand {
    pub action: CheckAction,          // M-00 冻结枚举
    pub program: String,
    pub argv: Vec<String>,
    pub timeout_secs: u32,            // 描述符模板显式声明
    pub template_sha256: Sha256,      // 与 Run 冻结描述符逐字节核验
}

#[serde(tag = "source", rename_all = "SCREAMING_SNAKE_CASE")]
pub enum OverlaySource {
    TestedCommit { tested_commit_oid: GitOid },
    BaselineInit,
}
...
pub struct OverlayGrant {
    ...
    pub root: SandboxPath,            // worker 侧唯一可见的源码副本挂载点
}
...
pub struct ExecuteCheck {
    ...
    pub command: FrozenCheckCommand,
    pub toolchain_image_digest: String,
    pub overlay: OverlayGrant,
}
```
...
## worker 返回先经过 active-attempt gate
...
## 每个 check 都使用一次性 validation overlay
...
`Shell` 工具（M-12）不经营任何命令面：模型提交自由命令文本（含参数），无白名单、无 `CheckCommandTemplate` 实例化、无 active-attempt gate，直接在本 Slice 专属长驻沙箱卷内执行。这是 Agent 的直接执行通道，与 UDS 派发链平行。

**物理形态：工作区即沙箱卷（M-08）。** 每 Slice 候选工作区就是一个沙箱卷：宿主 app 与该 Slice 专属沙箱共享挂载——受信侧（L1 结构化文件工具）与不可信侧（Shell 命令）操作同一文件系统的不同视图。沙箱生命周期与 Slice 对齐，而非与单次检查对齐：Slice 创建时建卷，集成 receipt 或废弃时销毁工作区与沙箱；裁决层检查仍在每次派发时从 `tested_commit_oid` 新建一次性 overlay，两者不共享隔离实例。

**驻留复用。** 构建缓存与已装依赖跨命令驻留在沙箱卷内：同会话重复构建/测试不重复下载与冷编译，迭代加速是长驻的主要收益。驻留缓存不进入验证输入身份——裁决层检查从冻结 tested commit 的一次性 overlay 执行，不读取沙箱卷内缓存。
...
**Shell 进程治理。** Shell 的 bwrap 进程组由 app 直接派生并持有：进程组置于专属 cgroup 域、bwrap 首进程设 `PR_SET_PDEATHSIG`——app 进程死亡时内核同步回收其全部 Shell 沙箱进程组，不依赖 worker 断连清理（Shell 不经 UDS 派发链，worker 无权也无需清理它们）；checkpoint quiesce 与会话失效复用同一 cgroup 域终止语义（M-08）。
...
Spec、API 与模型都不能表达 program、argv、shell、环境变量或 timeout。Harness 在派发前以冻结参数实例化目标端描述符的 `CheckCommandTemplate`（canonical 顺序固定），把完整命令与 `template_sha256` 一并发送；worker 重算摘要并核验目标工具链镜像摘要，不从仓库、模型或调用方补全 argv，也不经 shell 拼接。
...
| app | active dispatch set、被检内容、overlay grant、镜像摘要 | worker 返回 | 校验身份并决定接纳 |
| worker host | 已验镜像摘要、资源策略、argv、scoped CAS ingest grant | 检查进程行为 | namespace、cgroup、日志捕获、回收 |
...
| validation overlay grant | Run/subject/check/attempt 与预注册目录一致 | 只挂载按 grant 来源新建的一次性 overlay；不接受候选工作区、canonical ref 或任意路径 |

### bubblewrap 参数边界

| 顺序 | 参数类别 | 来源 | 安全作用 |
|---:|---|---|---|
| 1 | 固定 bubblewrap executable | worker image | 不从仓库找 launcher |
...
| 7 | validation overlay bind | overlay grant | 单次 check 唯一可写源码副本；候选工作区/canonical ref 不可见 |
...
UDS 运行目录、PostgreSQL socket、Docker socket、SSH agent 和宿主凭据都不在挂载表中。翻译后的 pytest 测试代码可以在沙箱内运行，但仍属于不可信进程，不能据此把任何检查器提升为受信 launcher。
...
沙箱内代码对控制面 socket 的探测事实归本节记录：bubblewrap 内代码探测任何宿主 socket 时，socket 未挂入 namespace，无控制请求产生，统一以 sandbox 权限拒绝审计呈现（自 M-03 断连表移交至此——它是沙箱出口探测事实，不是 app↔worker 断连方向）。
...
裁决层单次检查沙箱固定 `4 GiB` 内存、`2` CPU、`10 GiB` 可写磁盘。**三池模型**（M-00 信息分层原则/fb8 续对齐）：物理公式 `max(1, min(4, floor(host_memory_gib / 4), floor(host_cpu_cores / 2)))` 约束的是**同时活跃的 bwrap 沙箱执行位**——Shell 命令、Scaffold 与裁决层一次性检查按需从池中取用、用毕归还（长驻卷与构建缓存保留在卷内，不受归还影响）；Slice 会话数不占沙箱执行位——模型调用与只读工具在 app 进程内执行，其并发由 provider 配额与 Run 预算约束。`Exec` 嵌入式 JS 引擎在 app 进程内运行，不经沙箱：引擎实例有独立内存/CPU 上限，不占沙箱槽位；引擎选型（deno_core vs rquickjs 类）与开销基准为实施期细化项，以实测基准回填上限数字（M-12 联动）。计量口径定调：沙箱内存按 cgroup memory limit 以**活跃 bwrap 实例**保守计入并发内存预算（不按实测 RSS 折扣）；空闲治理方向——Slice 会话进入 `WAITING_FOR_USER` 等非活跃态超过阈值（实施期参数）时终止沙箱内进程组并归还池位、保留卷内容，构建缓存与已装依赖为文件态不受影响，恢复活动时重建进程组即可续用。[M-03](CodeMigrator_Harness总体设计.md) 在各池之上执行跨 Run 公平轮转，同一 Run 只派发互不冲突的 ready 键。
...
| overlay 内单文件写入 | 64 MiB | `OutputLimitExceeded` | quota receipt |
