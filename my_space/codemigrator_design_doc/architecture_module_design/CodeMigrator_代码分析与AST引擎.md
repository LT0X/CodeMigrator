...
> 文档状态：V6 收敛版（fb11，ANALYZE 职责并入 CreateRun，见下方 V6 演进说明）。  
...
> **V6 演进说明**：知识图谱 / PSF 投影的**首次构建前移**至项目注册 / 起草会话开启时（探索开始前必须可用），在冻结快照上即完成机械完备层管线——区别于 V5 将首次构建挂在 ANALYZE 之后。**V6 收敛**：原「ANALYZE 职责」不再作为独立阶段存在（RunStatus 时序收敛为 CREATED→PLANNING→EXECUTING→VERIFYING→REPORTING），其校验职责**并入 CreateRun**——复用预索引投影 + 冻结校验（投影描述符摘要与 Spec 锁定摘要的一致性断言）+ 档案一致性校验（UnderstandingDossier 与机械事实核对，不一致以 `DOSSIER_INCONSISTENT` fail-fast 拒绝），CreateRun 后不再重复解析、不复现第二次解析。V5 的机械完备层、PSF 三层模型与确定性派生保持，仅语义校准发生在输入/基线与草案之间而非解析重复；项目注册时构建的那份投影亦是完备性对抗审计的作用对象（见「完备性对抗审计」节）。V5 当前对齐段留存在下节作为追溯。
...
机械完备层仍由 tree-sitter/CST、清单解析、符号索引和关系图承担；语义模块边界不再由机械层直接授权。PSF-3 关系图是多会话探索和 Planner 的只读共享媒介，至少支持符号搜索、caller/callee、影响范围、上下文检索与模糊全文检索；探索报告必须以 file:range 锚点和置信度理由归因。保留双怀疑派审计、双向 diff、最多三轮修规则不修结果和覆盖率自述。**V6 不推翻本节**：机械完备层、PSF 三层模型与确定性派生全部保持；V6 仅将首次构建时点前移至项目注册/起草会话开启，并把原「复用 + 校验」职责并入 CreateRun（不再存在独立 ANALYZE 阶段，见「时序重定义」节）。

## 完备性对抗审计：机械层的运行时质量闭环

金标准 fixture（V-M06-V4-019）验证**提取规则**的正确性；对抗审计验证**每一次运行**的产出质量。审计作用在**项目注册时构建的那份投影**（预索引）之上，发生时点=**项目注册/预索引时（签核前）**对冻结投影执行（fb11 对齐：作用在项目注册投影上而非法二次解析；V6 无独立 ANALYZE 阶段——ANALYZE 已并入 CreateRun，对抗审计**不在 CreateRun 预检点重跑**、预索引后不发生第二次解析）。Anthropic 迁移 kit 同款流程实证——"地图与缺口清单的发现来自对抗评审，而非初稿"：
...
## 注册预索引产出的四类事实（原"ANALYZE 产出"语义，随 CreateRun 预检保留）
...
**ANALYZE 职责并入 CreateRun**（V6 收敛，不再存在独立 ANALYZE 阶段）：原 ANALYZE 的校验职责统一归入 CreateRun 的校验前置，CreateRun 后不重跑机械完备层、不产生第二次解析：

1. **复用既有投影**：直接定位/装载项目注册时预索引构建的投影（不重复解析）。
2. **冻结校验**：投影描述符摘要与 Spec 锁定描述符摘要一致性**断言**（能力门不在 CreateRun 前重复解析，仅断言）。
3. **档案一致性校验**：与用户确认后的 UnderstandingDossier 做机械事实一致性核对；校验不一致以 `DOSSIER_INCONSISTENT` **fail-fast 拒绝**该 Run，不进入 PLANNING；偏差记入分析投影、不回写工件。

RunStatus 因此不再有独立 ANALYZE 阶段，时序为 CREATED→PLANNING→EXECUTING→VERIFYING→REPORTING。
...
| 分析时点 | V6：机械完备层在探索开始前（项目注册/起草会话开启）对冻结快照完成；原 ANALYZE 职责并入 CreateRun（复用投影 + 冻结校验 + 档案一致性校验，fail-fast），无独立 ANALYZE 阶段，不再重复解析 |
