...
> 技术范围：Rust 2024、单机 Docker Compose、双工具链描述符、Linux sandbox worker、PostgreSQL 控制面。  
...
| crate 清单重组、目录架构与本地 worker 协议归属 | [M-01](CodeMigrator_核心目录架构设计.md) | M-03、M-05、M-06、M-09、M-12 |
...
核心 crate 清单由 [M-01](CodeMigrator_核心目录架构设计.md) 按 V4 重组后重新冻结，本篇不写死数字，只锁定两条边界：核心不依赖具体语言对与描述符内容；本地 worker 协议属于 `codemigrator-sandbox`。默认 Compose 只有 `app + sandbox-worker + PostgreSQL`：app 拥有 API、Run actor、计划与集成；worker 只通过宿主可访问的 Unix domain socket 接受类型化执行请求；bubblewrap 子进程看不到控制 socket，worker 也没有数据库凭据。源项目快照以只读挂载进入沙箱；目标输出是受 Git refs 管理的托管输出工作区，不是用户分支直写。
...
下列声明位于 `codemigrator-core`。所有 `RunId/SpecId/SliceId/TaskId/CheckId/ReceiptId/RequestId/DispatchAttemptId` 都是 UUID v7 newtype：JSON 使用小写连字符字符串，SQL 使用 `UUID`。`CandidateGeneration` 是受验证的 `u8` newtype，只允许 `0..=2`；`0` 是初始候选，`1` 与 `2` 是基于最新 verified 的两次语义重生成，物理重派不会增加 generation。

```rust
pub struct BranchPrefix(String);
...
pub struct CandidateGeneration(u8);
pub struct DispatchAttemptId(uuid::Uuid);
pub struct DeterministicPlanOrderKey(Sha256);
pub struct SessionId(uuid::Uuid);
pub struct MessageId(uuid::Uuid);
pub struct QuestionId(uuid::Uuid);
pub struct TaskDraftRevisionId(uuid::Uuid);
pub struct CorrectionIntentId(uuid::Uuid);
pub struct PlanRevisionId(uuid::Uuid);
pub struct ProjectId(uuid::Uuid);
pub struct ProjectSnapshotId(uuid::Uuid);
pub struct OutputWorkspaceId(uuid::Uuid);
pub struct ProjectModuleId(uuid::Uuid);

pub enum MigrationSessionStatus { Drafting, ReadyToConfirm, AttachedToRun, Closed }
/// InteractionStatus 辨析收敛：PausingForInput＝正在排空至安全点的瞬态（在途原子操作计数由 Run 投影表达）；WaitingForUser＝已抵达安全点、等待用户输入的稳态。
pub enum InteractionStatus { Ready, PausingForInput, WaitingForUser, ApplyingCorrection }
...
pub enum CorrectionIntentStatus {
    Received, NeedsClarification, NeedsConfirmation,
    Accepted, Applied, DeferredToFollowUp, Rejected,
}

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum SliceKind { Contract, Implementation, TestTranslation, TestGeneration }

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum ArtifactKind { GeneratedCode, DeclarativeConfig, ResourceFile }

/// 《项目理解档案》：起草会话深潜阶段（理解会话本体）产出、用户确认后冻结为 Run 输入的一等公民工件。
/// 全部条目必带 file:range 锚点（CodeAnchor），不可锚定的叙述显式标记 advisory。
pub struct UnderstandingDossier {
    pub architecture_narrative: Vec<DossierEntry>,        // 架构叙事与分层
    pub semantic_modules: Vec<DossierEntry>,              // 语义模块划分建议（成员文件清单，可修正目录约定）
    pub dependency_resolutions: Vec<DossierEntry>,        // 对机械候选边/Unknown 的判定：真实依赖/误报/隐式依赖 + 置信理由
    pub test_map: Vec<DossierEntry>,                      // 测试地图（测试组织与覆盖叙事）
    pub risk_hotspots: Vec<DossierEntry>,                 // 迁移风险热点（动态 import/反射/全局态等）
    pub strategy_advice: Vec<DossierEntry>,               // 迁移策略建议（批次/顺序/框架惯用法）
    pub coverage_self_report: CoverageSelfReport,         // 探索覆盖率自述（触达目录/文件清单 + 未读区声明）
    pub budget_tier: DossierBudgetTier,
}

pub struct DossierEntry {
    pub kind: DossierEntryKind,
    pub content: String,                 // 叙述正文
    pub anchors: Vec<CodeAnchor>,        // file:range 锚点；advisory 条目可为空但须标注
    pub advisory: bool,                  // 无锚点的纯建议性条目
}

pub enum DossierBudgetTier { Shallow, Deep }

/// 《迁移规则手册》：起草会话产出并经用户确认冻结；Run 内经受控追加通道演进。
/// 规则是知识不是计划事实——追加不触发 PlanRevision，生效面=后续派发的会话。
pub struct MigrationRulebook {
    pub entries: Vec<RulebookEntry>,
    pub version: u32,                      // 随受控追加递增；每个会话 pack 记录其消费的版本号
}

pub struct RulebookEntry {
    pub kind: RulebookEntryKind,           // 惯用法映射/命名约定/错误处理模式/框架 idiom 等
    pub content: String,
    pub source: RuleEntrySource,           // DraftingSession（初版，用户确认）| AttributionProposal（归因驱动追加）
    pub rationale_ref: Option<ArtifactRef>,// 追加条目必填：指向归因诊断审计事实
    pub advisory: bool,
}

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum SliceAttemptStatus {
    Ready, Running, LocalVerifying, LocallyVerified,
    IntegrationQueued, Integrating, Regenerating,
    Integrated, TerminalFailed, Cancelled,
}

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum WriteScope {
    Out {
        write_paths: Vec<RepoRelativePath>,   // 枚举文件集：修改与新建权
        create_roots: Vec<RepoRelativePath>,  // 目标模块目录集合：仅新建权
    },
...
pub struct MigrationSlice {
    pub id: SliceId,
    pub kind: SliceKind,
    pub source_modules: Vec<ProjectModuleId>,
    pub write_scope: WriteScope,
    pub required_checks: Vec<RequiredCheck>,
    pub topological_layer: u32,
    pub deterministic_plan_order_key: DeterministicPlanOrderKey,
}

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum PlanEdgeKind { Requires, OrderedBefore }
...
pub struct SliceCandidate {
    ...
    pub base_verified_oid: GitOid,
    pub candidate_commit_oid: GitOid,
}

pub struct ActiveDispatch {
    pub dispatch_attempt_id: DispatchAttemptId,
    pub subject: ExecutionSubject,
    pub check_id: CheckId,
    pub tested_commit_oid: GitOid,
}

pub struct GitRunRefs {
    pub base_commit_oid: GitOid,
    pub verified_commit_oid: GitOid,
}

pub struct CreateRun {
    pub source: CreateRunSource,
    pub branch_prefix: BranchPrefix,
}

pub enum CreateRunSource {
    RemoteRepository { repository_url: RepositoryUrl, base_ref: GitRefName },
    RegisteredProject { project_id: ProjectId, snapshot_id: ProjectSnapshotId },
}

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum RunStatus {
    Created, Analyzing, Planning, Executing, Verifying, Reporting,
    Completed, PartiallyCompleted, Failed, Cancelled,
}

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum FailureReason {
    AnalysisFailed, PlanFailed, ExecutionFailed,
    VerificationTerminal, ReportGenerationFailed, BudgetExhausted,
    ResourceExhausted, OutputLimitExceeded,
    SliceRegenerationExhausted, NondeterministicVerification,
}

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum DeliveryChannelStatus { Pending, Generating, Ready, DeliveryFailed }  // 统一交付通道状态（报告/代码两通道共用；Generating 仅报告通道使用）

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum ModelProfile { Reasoning, Code }  // 挡位收敛：Fast 档删除——VERIFY 不开模型会话、REPORT 确定性模板拼装正文

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum Phase { Analyze, Plan, Execute, Verify, Report }
```
...
```rust
pub struct TreeSitterGrammarRef {
    pub grammar_id: String,
    pub grammar_sha256: Sha256,
}

pub struct ManifestParserRef {
...
pub struct SourceToolchain {
    ...
    pub parser: TreeSitterGrammarRef,
    pub manifest_parsers: Vec<ManifestParserRef>,
    pub module_boundary_strategy: ModuleBoundaryStrategy,
}

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum ModuleBoundaryStrategy {
    ManifestPerModule,                  // 每清单一个模块（npm workspace 型）
    SingleManifestDirectoryConvention,  // 单清单 + 目录约定划界（go.mod 型）
    DirectoryConvention,                // 无清单：纯目录约定
}

pub struct TargetToolchain {
    pub language_id: LanguageId,
    pub package_manager: String,
    pub scaffold: Vec<CheckCommandTemplate>,
    pub build: Vec<CheckCommandTemplate>,
    pub test: Vec<CheckCommandTemplate>,
    pub lint: Vec<CheckCommandTemplate>,
    pub typecheck: Vec<CheckCommandTemplate>,
    pub toolchain_image_digest: String,
    pub build_excludes: Vec<RepoRelativePath>, // 依赖副产物规范排除集：不计入 checkpoint diff 校验、不进 candidate commit
}

pub struct ToolchainDescriptor {
    pub descriptor_version: semver::Version,
    pub descriptor_sha256: Sha256,
    pub source: SourceToolchain,
    pub target: TargetToolchain,
}

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum CheckAction { Scaffold, Compile, Test, Lint, TypeCheck }

pub struct CheckCommandTemplate {
...
pub struct ContractArtifact {
    ...
    pub target_module_path: RepoRelativePath,
    pub public_signatures: Vec<String>,
    pub types_hash: Sha256,
}
...
pub enum DiagnosticTarget {
    FileLine { file_path: RepoRelativePath, line: u32 },
    TestIdentity { test_name: String },
    Unknown,
}
...
pub struct DiagnosticMapping {
    ...
    pub code: String,
    pub message_hash: Sha256,
}

#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum CheckStatus {
    Passed, Failed, TimedOut, OutputLimitExceeded, InfrastructureError,
}

pub struct CheckResult {
    pub check_id: CheckId,
    pub invocation_hash: Sha256,
    pub status: CheckStatus,
    pub receipt_id: ReceiptId,
    pub stdout: ArtifactRef,
    pub stderr: ArtifactRef,
    pub diagnostics: Vec<DiagnosticMapping>,
}
...
pub type ExecutionSubject = VerificationSubject;

pub struct VerificationOutcome {
    pub run_id: RunId,
    pub subject: VerificationSubject,
    pub tested_commit_oid: GitOid,
    pub frozen_required_checks_sha256: Sha256,
    pub check_results: Vec<CheckResult>,
    pub verification_fingerprint: Sha256,
}

pub struct DerivedVerificationGuard {
    pub all_required_checks_passed: bool,
    pub error_unknown_count: u32,
}
...
`VerificationSubject` 是判别联合：局部结果只能携带 Slice/generation/candidate OID，集成结果必须携带 Slice/generation/expected verified 与 prospective scratch OID，最终结果只携带冻结 verified OID；schema 对 variant 外字段执行 `deny_unknown_fields`。worker protocol 以 `ExecutionSubject` 类型别名直接复用这一判别联合，不另造弱化身份。`tested_commit_oid` 必须等于 subject 的 candidate、prospective scratch 或 verified OID。active dispatch 的唯一键为 `run_id + canonical(subject identity) + check_id`；一个 Run 可同时拥有多个键，每个键恰有一个 active `DispatchAttemptId`，返回还必须回显相等的 `tested_commit_oid`。
...
|---|---|---|---|---|
| 局部验证 | `VerificationSubject::LocalCandidate` 指向的 candidate commit | 描述符语法检查 + 对契约的类型检查模板；项目尚不完整，不跑全量编译 | 状态进入 `LOCALLY_VERIFIED/INTEGRATION_QUEUED` | 可修复问题在同 generation 内处理；终态问题进入重生成判断 |
| 集成验证 | `VerificationSubject::ProspectiveIntegration` 指向的 prospective commit | 增量全量：目标编译 + 已集成部分可运行测试 | 允许创建 `IntegrationIntent` | 接口冲突或可修复验证失败时，从最新 verified 创建下一 generation |
| 最终验证 | `VerificationSubject::FinalVerified` 指向的冻结 verified head | 目标端描述符 test 模板全集：翻译后全套测试 | 进入 `REPORTING` | 与最近相同 `tested_commit_oid + frozen_required_checks_sha256` 的 `ProspectiveIntegration` 语义 fingerprint 不一致时返回 `NONDETERMINISTIC_VERIFICATION`；一致时按归因规则定向重生成 |
...
工具 policy 由 `codemigrator-core` 以 `core://phase-tool-policy/v2` 编译时资源发布；工具箱方法集合与 frame 规则仍由 M-01 与 M-12 所有。`WriteFile/EditFile` 的写路径被限定在本 Slice 冻结 write scope 内，越界返回 `WRITE_SCOPE_VIOLATION`；`Shell` 自由执行于长驻沙箱，checkpoint 提交时批量校验工作区 Git diff 全落冻结 write scope，越界拒绝提交且不污染 verified；`Exec` 编排底层每次工具调用逐笔过网关；`ReadFile` 可读源项目快照、已集成契约与本 Slice 候选工作区。模型/agent 在 `VERIFY` 请求任何工具（包括 `ReadFile`、`QuerySourceAst` 与检查执行）均返回 `TOOL_PHASE_DENIED`。
