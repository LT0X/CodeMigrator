...
以上两者皆为运行投影的只读展示，不新增任何写通道；细节 UI 与所需投影归 Web 详细编码设计，于实施期细化。
...
| 全局修复决策 / 全局修复会话生命周期事件 | 不属原 Slice，状态不变 | 不作为 Slice 四场动作；新会话类型独立呈现位（可选默认呈现） | 全局修复集 / 联合域状态标识，标注"不属原 Slice" |
| Supervisor 已收养修复决策与路由结论（只读） | 状态不变 | EXECUTE 阶段只读决策视图，不驱动舞台动作 | 已收养修复决策与路由结论摘要 |

> 注：上表中 repair 相关两行属于**可选默认呈现**——它们只是全局修复会话与 Supervisor 决策视图的入流事件；实现时须提供可理解的默认渲染（见下文渲染完备性义务），但默认不插入 Slice 舞台分区，也不新增任何写通道。
...
- [ ] 全局修复会话作为新会话类型独立呈现，带全局修复集 / 联合域 / "不属原 Slice" 状态标识，不作为既有 Slice 或其 generation 呈现；其生命周期事件经 M-02 投影只读消费，前端零判断。
- [ ] EXECUTE Supervisor 决策视图仅呈现已收养的修复决策与路由结论，只读；不提供收养/否决/改派操作入口。
- [ ] 全局修复决策 / 修复会话生命周期事件与 Supervisor 决策事件提供可理解的可选默认呈现（独立会话呈现位 / EXECUTE 只读决策视图），不插入 Slice 舞台分区；Web 仍只读运行投影，未新增任何写通道。
