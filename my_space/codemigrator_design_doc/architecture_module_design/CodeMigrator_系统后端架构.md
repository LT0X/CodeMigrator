...
后端不把 HTTP 请求直接变成一次迁移执行。它的任务是把外部的跨语言翻译意图——四件冻结工件与一个 CreateRun——变成可恢复的 Run 账本，把 Planner 提案、机器校验、并行执行和 Oracle 运行事实投影成稳定 API，并把代码交付和报告交付与迁移结果分开。这样模型会话、bwrap 执行、可选 artifact 镜像、观测 profile 或外部投递失败时，浏览器仍能从 PostgreSQL 获得没有歧义的状态。
...
本轮 V5 对齐先声明三条不变骨架：SSE 投影机制（`migration.event` v1 信封、`next_event_sequence` 分配、`Last-Event-ID` 回放、NOTIFY 仅唤醒）不变；控制面事务边界（可观察投影与对应 `run_events` 在同一 PostgreSQL 事务提交、共同回滚）不变；API 幂等语义（`(principal_id, route, key)` 作用域、24 小时留存、canonical body 匹配规则）不变。调整集中在两处：外部 DTO 升级为跨语言语义（Spec v3、四类 Slice），`run_events` 事件集补充起草确认、Planner 校验、契约/执行、测试移植、归因与工具审计事件。

## V6 方向对齐

V6 把判断层协调语义纳入 `run_events` 事件集与投影：`run_events` 事件集新增 Advice/修复决策/全局修复会话生命周期事件，并补充对应的 SSE 与 REST 只读投影。这些事件沿用既有 `migration.event` v1 信封与 `(run_id, sequence)` 回放，不改变 envelope 版本；事件数据字段均经既有 redaction 出口。SSE 与 REST 在此基础上补充判断层建议的收养展示口径；Web 新增修复会话状态与 Supervisor 决策的只读呈现（详细 UI 归 M-15 拥有）。投影机制、控制面事务边界与 API 幂等语义三条骨架保持 V5 不变。

## V5 当前对齐

默认拓扑收敛为 app + PostgreSQL。后端需持久化四件冻结工件的 ref/hash、探索报告与覆盖聚合、PlanProposal、PlanValidation 和自动冻结结果，并通过同一 REST/SSE 投影提供起草确认、计划校验、Slice 运行、验证证据和 M-16 修正确认状态。旧 worker 健康状态、UDS 派发状态和 overlay 授权不再是 API 的运行事实；六工具审计、Oracle 证据、Git receipt、预算与事件回放继续保留。

## 系统上下文：app 与 PostgreSQL 完成迁移闭环
...
`app` 是 `codemigrator-server` 的部署单元，内部组装 API、runtime、Run actors、LLM Planner、内置工具链描述符资源账本、受信 Git 集成服务、host CAS、bwrap 生命周期、事件投影与事实驱动恢复；它拥有托管输出工作区、内部 Git refs 与大对象正文的访问权。bwrap 只看到 app 授予的长期 Slice 卷或验证临时目录，不连接 PostgreSQL、Git、host CAS 或控制面网络。PostgreSQL 是 Run、四件冻结工件、PlanProposal、PlanValidation、Slice、candidate generation、active dispatch、验证证据、integration intent/receipt、幂等记录、append-only `run_events` 和 artifact 引用的控制面真相源；Git 是代码与 commit 事实，host CAS 保存源快照内容、模型/工具日志、完整 stdout/stderr 与报告正文。可选 profile 只能提供镜像或诊断，关闭它们不能阻断默认迁移闭环。

| Compose 集合 | 服务 | 可用性语义 |
|---|---|---|
| 默认 | `app`、`postgres` | 二者任一未就绪时拒绝新 Run；已有状态仍从 PostgreSQL 读取 |
...
CreateRun 的 source 是判别联合（Pydantic v2 discriminated union）：`RemoteRepository` 固定为 `repository_url + base_ref`，`RegisteredProject` 固定为 `project_id + snapshot_id`；二者都与 `branch_prefix` 及四件已确认工件的 `FrozenArtifactBundle` 一起提交，不存在 `target_branch` 或客户端输出路径——跨语言翻译的输出全部落在托管输出工作区，不写回源仓库。`branch_prefix` 是已验证的 ASCII 输入，长度为 1 至 32 字节，只含小写字母、数字、`-` 和 `/`，拒绝空段、`.`、`..` 和 `.git`；交付分支命名形状由 [M-11](CodeMigrator_工作空间与Git集成.md) 拥有，内部 ref 一律以 `run_id` 组织（M-00）。
...
sequenceDiagram
    ...
    participant X as app-managed bwrap 执行位
...
| `GET /api/v1/migrations/{run_id}` | 读取四类状态投影 | 不触发 bwrap、模型或 Git 副作用 |
...
| `GET /api/v1/system/health` | 读取 app、PostgreSQL、三池和可选 profile 的安全摘要 | 不暴露 DSN、凭据、socket 或宿主路径 |
...
| `POST /api/v1/sessions/{session_id}/messages` | 持久化会话或运行中修正消息 | 不直接写 candidate、Git、bwrap 或数据库领域事实 |
...
外部 DTO 只投影已冻结事实，不发明第二套领域状态；内部类型一律引用 M-00 公共契约，本篇不复制定义。REST 投影模型统一以 Pydantic v2 `BaseModel` 表述，经 FastAPI 路由序列化为 JSON；字段名与语义以本篇为准。
...
class SpecView(BaseModel):
    ...
    target_language_id: LanguageId       # 跨语言对：必须与源语言不同
    descriptor_lock: DescriptorLockView
    required_checks: list[RequiredCheckView]
    # decomposition 透传 M-05 规范化正文，本篇不另造形状


class DescriptorLockView(BaseModel):
    descriptor_version: str              # 语义化版本
    source_descriptor_sha256: Sha256
    target_descriptor_sha256: Sha256
    toolchain_image_digest: str


class RequiredCheckView(BaseModel):
    action: CheckAction                  # Scaffold/Compile/Test/Lint/TypeCheck
    template_sha256: Sha256              # 命令正文唯一存在于目标端描述符模板
```
...
class SliceView(BaseModel):
    ...
    slice_id: SliceId
    kind: SliceKind                      # Contract | Implementation | TestTranslation | TestGeneration
    status: SliceAttemptStatus
    generation: CandidateGeneration
    write_scope: WriteScope              # Out { write_paths, create_roots }
    integration_rank: int
    integration_rank: int                # 冻结集成序位次（M-07 集成键的投影）
```
...
| `slices` | 增 `kind` 列（CONTRACT/IMPLEMENTATION/TEST_TRANSLATION/TEST_GENERATION）；`write_scope` 存规范化输出路径集合；`integration_rank`、`source_modules`、Planner/PlanValidation 引用 | Planner 提案经机器校验后冻结的计划事实 |
...
客户端必须同时看到 Run 状态、验证事实、报告交付和代码交付，但它们不是一个枚举的不同文字。Run 的 `REPORTING` 表示报告正文仍在生成；正文生成失败以 `FailureReason.ReportGenerationFailed`（冻结错误码异常层次成员）把 Run 置为 `FAILED`。只有 Run 已终态后的发布、镜像或外部投递失败，才能改变报告交付状态；push 或 PR 失败只能改变代码交付状态。两通道共用统一状态枚举 `DeliveryChannelStatus`（`Generating` 仅报告通道使用），交付 ledger 仍分立为 `report_delivery_status` 与 `code_delivery_status` 两列投影、互不影响。
...
SSE 事件的唯一信封为 `schema`、`version`、`type`、`data`、`sequence`、`timestamp_utc` 六个字段（v1 真相源口径），其中 `schema` 固定为 `migration.event`，`version` 固定为整数 `1`；`sequence` 与 SSE 协议层 `id` 同值（sequence 十进制表示），使分页与回放依赖的 `(run_id, sequence)` 在信封体内自洽。事件集在 V5 扩充不改变信封版本：只有信封结构变化才引入 v2，新增 `type` 不构成版本变更。Run 行拥有 `next_event_sequence`：所有会改变可观察投影的事务先锁定该 Run 行，读取当前值作为新事件 sequence，写入投影与 `run_events` 后将计数递增；任一步失败时投影、事件和计数共同回滚。数据库再以 `(run_id, sequence)` 唯一约束拒绝重复，因此单 Run 的已提交事件从初始值连续且无缺口。
...
`type` 按现行事件全集投影。若计划包含 Contract Slice，其生命周期即 `slice.status_changed`（`kind=CONTRACT`）序列；测试翻译 Slice 的派发与完成即其 `slice.status_changed`（`kind=TEST_TRANSLATION`，`RUNNING → LOCALLY_VERIFIED → INTEGRATED/TERMINAL_FAILED`）序列；`test.failure_attributed` 与 `test.flaky_observed` 对应 M-10 定义的 `TEST_FAILURE_ATTRIBUTED` 与 `FLAKY_TEST_OBSERVED` 账本事件。
...
| Contract Slice 收敛（可选派生投影） | `execute.contract_wave_completed` | 计划实际包含的 Contract Slice 已集成摘要 | Integration Coordinator（集成 receipt 汇总派生，纯观测、非调度事实） | M-15 等待区（观测展示）、M-13 |
...
| 判断层建议 | `advice.proposed` | 建议 kind、proposal_hash、约束内/边界类目摘要 | 判断层（supervisor 会话） | CLI/Web 建议收养呈现、M-13 |
| 判断层建议收养 | `advice.adopted` | proposal_hash、收养结果、影响摘要 | Run actor（邮箱收养） | CLI/Web 收养呈现、M-13 |
| 全局修复决策 | `repair.decision` | 修复会话、触发摘要、决策摘要 | 判断层（supervisor 会话） | Web 决策只读呈现、REPORT |
| 修复会话生命周期 | `repair.session.started` / `repair.session.completed` | 修复会话 id、起止摘要、关联 slice | 判断层（supervisor 会话） | Web 修复会话状态、M-13 |

`execute.contract_wave_completed` 仅在计划实际包含 Contract Slice 时作为集成账本的派生投影——"计划内契约 Slice 已正式集成"这一事实由集成 receipt 汇总得出，不是新的波次状态，与 M-07"波次由 DAG 表达，不由状态机表达"一致；该事件仅承载观测语义，不作为任何 Slice 启动、派发或验收的时序依据——Slice 就绪唯一判据是冻结 DAG 的实际依赖闭包。无 Contract Slice 的计划不产生该事件。工具审计事件与 M-12 的网关点位同事务投影：每次调用的工具名、参数摘要与结果状态进入事件流，正文与路径原文不进入。

本轮 V6 新增的判断层事件（`advice.proposed`、`advice.adopted`、`repair.decision` 与修复会话生命周期事件）沿用同一信封与既有 redaction，data 字段均脱敏。`advice.proposed` 区分约束内与边界性建议：约束内建议的收养面向 CLI/Web 以采纳/忽略方式呈现，边界性建议以显式确认提示呈现；`advice.adopted` 由 Run actor 邮箱收养并记录收养结果。`repair.decision` 承载全局修复决策摘要；全局修复会话生命周期以 `repair.session.started/completed` 表达，实施期亦可并入既有 slice/修复会话状态（开放项）。SSE 与 REST 对上述事件提供只读投影用于上述展示口径。本表所列事件 data 关键字段仅为投影要点摘要，不构成 DTO/事件字段的完整 schema——具体字段形状与完整 DTO 属实施期开放项，本篇不臆造第二套定义。
...
因此 PostgreSQL 是唯一控制面后端，filesystem/JSONL 不进入 feature flag、repository Protocol 实现或测试矩阵。Git 继续只保存代码与 ref 事实，host CAS 继续只保存大对象正文；二者不能代替查询与事件账本。

## 贯穿示例：一次可恢复的 TS→Python 翻译

以下示例假设 Planner 选择了 Contract Slice；它用于说明事件投影和恢复，不构成 V5 的固定规划拓扑。
...
| CreateRun | source 判别联合、`branch_prefix` 与 `frozen_artifacts` 四件工件引用均完整；unknown、缺失或两个 source 同时出现均拒绝 | `422` |
...
- [ ] CreateRun 同时接收并冻结四件工件的 ref/hash；描述符资源与工件完整性预检失败时不分配 `run_id`，控制面、事件和 Git 无新增副作用。
- [ ] app + PostgreSQL 拓扑持久化起草报告、Blueprint、PlanProposal、PlanValidation、Slice、验证证据与事件；旧 worker/UDS/overlay 不作为 API 运行事实。
- [ ] API/SSE 只投影 actor 已提交事实；Planner 提案、机器校验、实际 DAG ready、三池等待、bwrap receipt 与 M-16 ImpactPreview 均可回放，Contract Slice 可不存在。
- [ ] `run_events` 与状态投影在同一事务提交；取消、迟到 dispatch、集成 receipt、交付失败分别归约，不互相改写迁移真相。
...
- [ ] V-M02-V4-012：全文档与路由表扫描零残留——不存在 `/api/v1/plugins`、`CAPABILITY_INCOMPATABLE`、`BuildInvocation`、edit intent、`GuardedPatch`、precondition/replacement/anchor/content 哈希或同语言迁移场景表述。
