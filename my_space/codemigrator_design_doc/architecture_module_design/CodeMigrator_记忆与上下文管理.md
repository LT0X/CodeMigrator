...
> 文档状态：V6 方向对齐版（fb11 收敛）；本篇是**全类型会话的统一上下文基建**——管理"会话如何装配、以及在预算内如何运转"，覆盖全部会话类型（交互式起草会话 / 工作 Slice 会话 / 协调 Supervisor 会话 / 修复会话），是本篇 Context Pack 装配与冻结、运行期数据块边界与中断会话重建的唯一 owner。会话差异只体现在装配模板与预算档，不分类型各搞一套压缩/管理实现。  
> V6 收敛（fb11）：协调 Supervisor / 探索协调者改为**事件触发式新会话（无常驻）**，随之取消常驻会话特有的滚动摘要机制——Supervisor 上下文为触发时定向装配（基线态势快照机器算、本身不入上下文 + 本次决策定向事件投影）；装配统一采用**三段式前缀**（稳定前缀段 / 演进段 / 定向增量段）最大化 provider prefix cache 命中；SessionKind 枚举与预算档保留，会话差异只在装配模板与预算档。全局修复会话改为导航索引式装配。"无自由记忆"原则精确化为"主 Agent 记忆 = 审计事实的结构化投影（可从 run_events 事件流重建），不含自由对话史"。V4/V5 已建立的预算治理、数据块边界、外置存储、逐出与中断重建机制保持，V5 对齐段保留作追溯基线。  
...
在 V5 对齐基础上收敛并补齐四类机制：其一，**会话预算档统一化**——SessionKind 枚举与各 `SessionBudgetProfile` 档位保留，但不分会话类型各搞一套压缩/管理实现，会话差异只在装配模板与预算档数值；其二，**协调会话事件触发式新会话**——探索协调者 / EXECUTE Supervisor 改为事件触发式新会话（无常驻），常驻会话滚动摘要机制取消，Supervisor 触发时注入基线态势快照（机器计算，本身不入上下文）与本次决策定向事件投影；其三，**三段式前缀装配**——稳定前缀段（角色系统提示 + 冻结工件引用 + Run 级固定事实）+ 演进段（verified 演进摘要，每集成一 Slice 仅在段尾增量追加自身条目、历史段零变动）+ 定向增量段（本次失败证据），由统一装配器承载，逐出只作用于定向增量段内旧轮次工具结果，稳定前缀段 + 演进段纳入不可逐出集合保护前缀字节稳定性；其四，**修复会话导航索引式装配**——全局修复会话初始装配不再一次性装满全境，修复简报为必要输入（不可静默截断），其余以导航索引形态给出（涉及文件 + 位置清单），Agent 按需 ReadFile 拉取，超限部分外置 host CAS 并经 cas:// 取回。V4/V5 的预算治理、数据块边界、外置存储与中断重建机制原样保持，V5 对齐段留存作追溯基线。
...
class SessionKind(str, Enum):
    ...
    # V6 收敛：协调级会话（事件触发式新会话，跨 Slice 协调，非单一 Slice 专属）
...
## V6 增量机制：统一上下文装配、事件触发式协调会话与修复会话导航装配

### 会话类型与预算档统一化：差异只在装配模板与预算档

`SessionKind` 枚举与每档 `SessionBudgetProfile`（`initial_pack_token_cap` / `session_token_cap` / `eviction_watermark_pct`）保留，但机制描述统一：全部会话类型共享同一套上下文装配、逐出与 KV-cache 前缀机制，**不分类型各搞一套压缩/管理实现**。会话跨类型的差异只体现在两点——**装配模板**（角色系统提示与内容构成）与**预算档数值**。协调级会话（探索协调者 / EXECUTE Supervisor）与修复会话由此沿用与工作 Slice 会话相同的装配器与预算治理，只是以各自模板与档位实例化。

### 事件触发式协调会话（无常驻，滚动摘要取消）

探索协调者（探索期协调多路探索，EXECUTE 阶段不参与）与 EXECUTE Supervisor（EXECUTE 阶段全局协调与修复编排）改为**事件触发式新会话**：不存在跨触发连续、常驻观察的会话实例，**常驻会话与其滚动摘要机制取消**。二者不针对单一 Slice，故不携带 `SliceGenerationRef`；会话类型构成定义归 M-04，本篇只叠加预算档——二者计入模型会话池（受 Run 的会话配额治理），并以独立 `SessionBudgetProfile` 档位实例化（档位数值为实施期基准，由版本化配置资源给出并随 Run 创建冻结，对齐 M-00 Q-V4-003）。

Supervisor 被触发时装配一段**定向上下文**：
...
定向投影由 Harness 从 `run_events` 审计事实确定性派生、可回溯审计事件，不构成自由记忆、不含对话史。会话触发即装配、结束即终止，无持久观察状态。

### 三段式 KV-cache 前缀架构

装配采用**三段式前缀**，以最大化 provider prefix cache 命中（恒定前缀段字节不变，缓存可命中免重计算）：

1. **稳定前缀段**：角色系统提示 + 冻结工件引用 + Run 级固定事实（spec hash / phase policy / 契约引用集合 / rulebook 版本等）。字节在 Run 生命周期内保持绝对稳定。
2. **演进段**：verified 演进摘要。每集成一个 Slice 只在段尾**增量追加**该 Slice 自身的摘要条目；"新来的 Slice"集成时仅追加自身条目，**历史段字节零变动**，段内既有条目不被改写。
3. **定向增量段**：本次失败证据 / 定向投影，每次触发不同，位于最末。

三段全部由本 Context Manager 的统一装配器承载——装配器按"稳定前缀 + 演进段 + 定向增量段"三层串接，不因会话类型而异。

**逐出交叉约束**：逐出只作用于**定向增量段内**的旧轮次工具结果；**稳定前缀段 + 演进段纳入"不可逐出集合"保护**——长会话逐出不得破坏前缀字节稳定性（前缀一旦被逐出改写将击穿 provider prefix cache，且演进段中的历史条目是已冻结审计事实、以字节稳定供前缀缓存复用）。必要输入不可静默截断规则（V-M14-V4-004）对稳定前缀段内系统提示 / 契约块 / 约定块继续适用。

> 实施期开放项：演进段追加字节稳定性的持久化保证（CAS 化落盘）、provider 侧 prefix cache 命中差异的适配与端到端命中率度量，均为实施期基准项，不以本篇冻结为设备契约。

### 会话创建模板库

Context Manager 协同 Harness 的**静态会话模板库**：角色系统提示来自 Harness 版本化受信模板库（随 rulebook / phase policy 同类受控并版本冻结，见 M-12），上下文内容段由本装配器确定性装配，Supervisor 修复简报按需注入。每种会话类型对应一个模板；模板库不是模型工具、不经 ToolGateway 工具面。
...
无自由记忆（精确化）：主 Agent 记忆 = 审计事实的结构化投影——任何"记忆"都可由 `run_events` 事件流确定性重建，**不含自由对话史**。不存在跨 Run 学习结论、偏好或历史对话的持久化；会话不跨 Run 学习结论——规则手册的归因驱动追加通道保持不变，结论只能经该受控通道进入规则手册，而非由会话自发生长。pack 与会话审计不包含上一会话的对话历史（RecoveryBrief 的审计摘要除外）。会话重建语义对协调级会话同样成立：事件触发式新会话的无常驻观察状态可由 `run_events` 事件流与定向装配完备重建，不依赖对话历史。M-16 的会话消息以脱敏、结构化的目标/禁止项/验收要求进入受限上下文，Skill 目录只作为上下文选择输入参与各阶段——其中嵌入的工具、shell 与 hook 指令一律忽略并记录（M-04/M-16）。
...
- [ ] 协调级会话（探索协调者 / EXECUTE Supervisor）为事件触发式新会话，具备独立 `SessionBudgetProfile` 档位；计入模型会话池且受独立预算档约束；档位数值实施期基准待版本化配置资源定案后随 Run 创建冻结。
- [ ] 协调级会话无常驻上下文：两次触发之间消费事件流条目数为 0；触发时定向装配基线态势快照（Harness 机器确定性计算、本身不入上下文）+ 本次决策定向事件投影（非全量事件流）；两者均可回溯 `run_events`。
- [ ] 统一装配采用三段式前缀（稳定前缀段 / 演进段 / 定向增量段）：演进段每集成一 Slice 仅在段尾增量追加自身摘要条目、历史段字节零变动；三段由统一装配器承载，不分会话类型各搞一套实现。
- [ ] 逐出仅作用于定向增量段内旧轮次工具结果；稳定前缀段 + 演进段未被逐出或改写，长会话逐出不破坏前缀字节稳定性。
- [ ] 会话创建模板：角色系统提示来自 Harness 版本化受信模板库（与 phase policy 同类管理），每种会话类型一个模板；上下文内容段由统一装配器确定性装配；模板库不经 ToolGateway 工具面。
