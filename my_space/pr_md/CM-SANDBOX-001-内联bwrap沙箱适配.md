...
- `src/codemigrator/sandbox/`：新增冻结 `CheckCommandTemplate` 的 `FrozenCommand`、独立 Shell `ShellCommand`、固定 bwrap argv、路径/环境/挂载安全门、镜像/模板摘要 gate、启动预检、PDEATHSIG、可选委派 cgroup、进程组回收、临时验证目录、固定资源策略、三池执行位、终止事实归约、执行回执和 asyncio 前向代理。
- `tests/sandbox/`、`tests/security/`：新增 argv 快照、摘要 mismatch 零启动、路径/环境、预检、生命周期、池公式、输出/终止、Shell/裁决分离、代理白名单/转发和审计测试。
- `my_space/`：新增实施计划、详细设计、迭代记录与本 PR 说明；主任务表同步 CM-SANDBOX-001 状态。
...
- `uv run --offline --frozen --group dev pytest -q` → `167 passed`
- `uv run --offline --frozen --group dev pytest -q tests/sandbox tests/security` → `16 passed`
...
- 执行器只负责事实回执；调用方必须提供已核验的模板/镜像摘要和（若启用资源治理）委派 cgroup，不能据此直接接纳验证结果。
