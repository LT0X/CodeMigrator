...
**归因禁猜硬边界**：只有查表**唯一命中**才触发定向重生成——多义命中经守恒信号辅助排序后仍多义的，进入所在层 Run 级终态兜底；禁止任何"按顺序默认取第一个候选"式猜测型 fallback（无证据的猜测型归属会错杀无辜 Slice，实测曾致三代误重生成 ≈35min，教训成文为硬边界）。
...
| 模糊但守恒事实不可用（源侧基线为 0 或覆盖状态 Undetermined），或辅助归因后仍多义 | — | 维持既有 Run 级终态兜底 |
...
```mermaid
flowchart TB
    D["DiagnosticMapping 诊断"] -->|FileLine| W["write scope 查表\nM-07 冻结映射"]
    W -->|唯一命中| OS["owning Slice"]
    W -->|无命中 / 多义| X["不可归因\n进入层终态路径"]
...
**超时处理**：check 级 `TimedOut`（模板 timeout_secs 到期被终止）不进入 flaky 重跑——超时是被终止的资源事实，不是断言语义失败。非 Test action 的 TimedOut 按运行级终态处理；Test action 的 TimedOut 若已收集的部分进度能定位在途测试身份，则按测试归因规则归因并定向重生成（翻译引入的死循环/挂起是真实迁移缺陷），无法定位时转入守恒信号辅助归因（见诊断归因节），仍无法判定才进入 Run 终态判断。
...
| 优先级 | 事实 | 唯一分类 |
|---:|---|---|
| 1 | `TimedOut`、`OutputLimitExceeded`、`InfrastructureError`（Test TimedOut 且可定位在途测试者、或失败证据模糊经守恒辅助归因唯一定向者除外） | `RUN_TERMINAL`，Run 失败（FailureReason 由 Harness 归约） |
...
| ProspectiveIntegration | 无法唯一归属 | 该 Slice 终态归约（`IndependentSliceTerminalFailure` 判定） |
| FinalVerified | 唯一归属 owning Slice | VERIFYING 内定向重生成（见下节），RunStatus 不变 |
| FinalVerified | 无法唯一归属（含守恒信号辅助归因三分支后仍无法判定） | Run 终态：`VerificationTerminal` → `FAILED`，不返回 EXECUTING |
...
3. 一致或不存在可比 outcome → 按 Final 的 exact-set、status 与 UNKNOWN 解释普通通过/失败。通过 → REPORTING；普通失败 → P-09 测试归因（符号级主路径，符号级解析失败时降级文件级两步归因）→ 唯一归属 → owning Slice 定向重生成；无法归属且失败证据模糊 → 守恒信号辅助归因三分支（离群 → 测试翻译 Slice / 无离群 → 实现 Slice / 仍无法判定 → Run 终态）；无法归属且证据清晰 → Run 终态判断。
...
```rust
pub struct StructuralConservationFacts {
    pub per_module: Vec<ModuleConservation>,
}

pub struct ModuleConservation {
    pub module: ProjectModuleId,
    pub test_ratio: f64,       // translated_tests / source_tests
    pub assertion_ratio: f64,  // translated_assertions / source_assertions
    pub loc_ratio: f64,        // translated_loc / source_loc
    pub outlier: bool,         // 任一比值超出 [0.5, 2.0] 带宽即标记
}
```
