...
上传正文为 UTF-8 JSON，最大 `256 KiB`（`262144` bytes），无 BOM、无重复 key、最大嵌套深度 `32`。顶层 `schema` 固定为 `codemigrator.migration-spec`，`version` 固定为整数 `3`，所有层级拒绝未知字段。最多返回 `100` 个按 JSON Pointer 升序排列的问题；其余以 `truncated=true` 标识。
