...
async def test_dispatch_gate_has_one_active_attempt_and_late_results_are_audit_only(run_id):
    ...
    await actor.submit(
        ExecutionReceiptMessage(run_id=run_id, dispatch=late, result_status="PASSED")
    )
    await actor.join()
...
async def test_matching_receipt_closes_gate_but_does_not_decide_verification(run_id):
    ...
    assert await actor.dispatch_started(active) is True
    await actor.submit(
        ExecutionReceiptMessage(run_id=run_id, dispatch=active, result_status="PASSED")
    )
    await actor.join()
    snapshot = await store.snapshot(run_id)
...
@pytest.mark.asyncio
async def test_misrouted_receipt_is_rejected_by_explicit_run_identity(run_id):
    store = InMemoryRuntimeStore()
    actor = RunActor(run_id, store)
    await actor.start()
    await actor.create(create_run())
    ...
    snapshot = await store.snapshot(run_id)
    assert snapshot.state.active_dispatches == (active,)
    assert snapshot.events[-1].data["reason"] == "RUN_ID_MISMATCH"
    await actor.stop()
