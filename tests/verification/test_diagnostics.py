...
def test_static_unique_attribution_is_reliable_and_warning_does_not_route() -> None:
    ...
    result = attribute_diagnostics(
        diagnostics,
        AttributionContext(
            write_scopes={slice_id: {"src/app.py"}},
...
def test_unknown_error_blocks_reliable_attribution_even_with_known_candidates() -> None:
    registry = DiagnosticParserRegistry.with_builtins()
    diagnostics = registry.parse(
        CheckAction.TypeCheck,
        "mypy",
        "src/app.py:3: error: bad type [assignment]\nopaque compiler payload",
    )
    slice_id = SliceId("00000000-0000-0000-0000-000000000001")
    result = attribute_diagnostics(
        diagnostics,
    ...
    assert result.candidate_slice_set == (slice_id,)
    assert result.reliability.value == "Uncertain"
    assert result.unknown_error_count == 1
    assert result.to_repair_evidence().conservation_signal_summary["error_unknown_count"] == 1


def test_partial_parser_output_is_explicitly_unknown() -> None:
    registry = DiagnosticParserRegistry.with_builtins()
    diagnostics = registry.parse(
        CheckAction.Lint,
        "ruff",
    ...
    assert any(diagnostic.target.kind == "UNKNOWN" for diagnostic in diagnostics)


def test_strong_coupling_requires_current_failure_hits_and_complete_facts() -> None:
    registry = DiagnosticParserRegistry.with_builtins()
    diagnostics = registry.parse(CheckAction.Compile, "go", "other.go:9: undefined: Value")
    first = SliceId("00000000-0000-0000-0000-000000000001")
    second = SliceId("00000000-0000-0000-0000-000000000002")
    result = attribute_diagnostics(
        diagnostics,
    ...
    assert result.strong_coupling is False
    assert result.reliability.value == "Uncertain"


def test_missing_coupling_facts_fail_closed_for_static_direct_routing() -> None:
    registry = DiagnosticParserRegistry.with_builtins()
    diagnostics = registry.parse(CheckAction.Lint, "ruff", "src/app.py:3:1: F401 unused import")
    slice_id = SliceId("00000000-0000-0000-0000-000000000001")
    result = attribute_diagnostics(
